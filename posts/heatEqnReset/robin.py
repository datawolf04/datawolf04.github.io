...
def plotRobinRod(kappa_value=1.0):
    t = 0 
    ...
    h = Constant(domain, PETSc.ScalarType(1))  # Robin boundary condition coefficient
    kappa = Constant(domain, PETSc.ScalarType(kappa_value))  # Thermal conductivity
    ...
    L = (uPrev + dt * f) * v * dx + dt * h * s * v * ds  # Linear form for the RBC
    ...
    fig, ax = plt.subplots(subplot_kw={'projection': '3d'},figsize=(8,6) ) # Create a 3D plot
    ax.plot_surface(Xc, Tc, Uc, cmap='viridis', edgecolor='none')  # Plot the FENICS solution
    ax.set_title(rf'$\kappa =$ {kappa.value:.1f}')
